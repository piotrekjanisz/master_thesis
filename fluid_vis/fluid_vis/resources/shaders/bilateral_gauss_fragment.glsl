...
uniform sampler2D gaussianDist;
...
void main(void)
{
	...
	ivec2 filter_size = textureSize(gaussianDist, 0);
	ivec2 half_filter_size = filter_size / 2;
	...
	for (int i = 0; i < filter_size.x; i++) {
		for (int j = 0; j < filter_size.y; j++) {
			vec2 coord = tex_coord + coordStep * (vec2(i, j) - half_filter_size);
			coord = clamp(coord, vec2(0.0, 0.0), vec2(1.0, 1.0));
			float sampleVal = texture(inputImage, coord).x;
			float w = texelFetch(gaussianDist, ivec2(i, j), 0).x;
